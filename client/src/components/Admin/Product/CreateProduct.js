...
const ProductForm = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setFormData({ ...formData, categoryId: response.data.data.items[0].id });
      const discountResponse = await api.get("discounts");
      setDiscount(discountResponse.data.data.items);
      setFormData({
        ...formData,
        discountId: discountResponse.data.data.items[0].id,
      });
    } catch (error) {
  ...
  const handleInputChange = (e) => {
    ...
    setFormData((prev) => {
      // Kiểm tra nếu trường được thay đổi là "discountId"
      if (id === "discountId") {
        // Tìm discount dựa trên value
        ...
        return {
          ...prev,
          [id]: value,
          discountPercent: selectedDiscount ? selectedDiscount.percent : 0, // Thêm discountPercent
        };
      }
