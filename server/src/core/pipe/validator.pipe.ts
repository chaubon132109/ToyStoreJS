...
export class ValidationPipe implements PipeTransform<any> {
  ...
  private async getMessage(
    ...
    if (!error.children || !error.children.length) {
      ...
      const property = error.property; // No translation, just use the property directly

      // Find the matching pattern.
      let patternFound: string | undefined;
      ...
      // Replace the constraint values back to the {constraintX} words.
      let errorMessage = constraint;
      const replacements = { property };

      if (match) {
        for (let i = 1; i < match.length; i += 1) {
          errorMessage = errorMessage.replace(match[i], `{constraint${i}}`);
      ...
      return patternFound
        ? `Validation failed: ${patternFound} for ${property}` // Custom message for pattern found
        : `Validation error: ${constraint}`; // Default message for other constraints
    }

    return this.getMessage(error.children, lang); // Recursion for children
  }
}
